...
export const useBlogForm = ({
  ...
  const handleImageChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>, index: number = -1) => {
  ...
  const prepareBlogData = useCallback(() => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      if (imageFiles[i]) {
        ...
        isEdit &&
        imagesToRemove[imageKey as keyof typeof imagesToRemove]
      ) {
      ...
      <body>
        ${
          formData.featuredImageUrl
            ? `<img src="${formData.featuredImageUrl}" alt="${formData.title}" class="featured-image" />`
            : ""
        }
