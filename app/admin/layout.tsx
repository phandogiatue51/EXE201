...
export default function AdminLayout({
  ...
  useEffect(() => {
    // Check admin authentication using centralized auth hook
    const checkAdmin = async () => {
      const current = await fetchCurrentUser();
      const role = (current?.role || localStorage.getItem("userRole") || "").toString().toLowerCase();
  ...
  const handleLogout = () => {
    // delegate logout to auth hook (it clears storage/state)
    logout();
    router.push("/login");
  };
